...
public class GuiMantenimientoAgregarCliente extends JDialog implements ActionListener, KeyListener {
	...
	public static void main(String[] args) {
		try {
			GuiMantenimientoAgregarCliente dialog = new GuiMantenimientoAgregarCliente(null);
	 ...
	 * Create the dialog.
	 */
	public GuiMantenimientoAgregarCliente(JDialog gcc) {
		super(gcc, true);
		setResizable(false);
		setTitle("Nuevo");
		...
		lblNewLabel.setBounds(10, 79, 126, 14);
		...
		lblStockMaximo.setBounds(352, 262, 126, 14);
		panelClientes.add(lblStockMaximo);
		
		txtDNI = new JTextField();
		txtDNI.addKeyListener(this);
		txtDNI.setColumns(10);
		txtDNI.setBounds(352, 280, 312, 31);
		...
		lblTitulo.setBounds(217, 11, 241, 42);
		panelClientes.add(lblTitulo);
	...
	private void agregar() {
		try {
			if(validarCamposVacios() && validarDNI() && validarTelefono()) {
				...
				ArregloClientes.ingresarClientes(
						new Cliente(codigo, nombres, apellidos, telefono, dni)
				);
				
				Utilidades.informationMensaje(this, "CLIENTE AGREGADO");
				dispose();
			}	
		} catch (Exception e) {
